...
Bits:
		...
		on file mapping
					...
					46-55 hold the idnex into the pages				//max... a lot of pages? too many to allocate in one go actually
...
//the deal is that instead of passing the pagetree poitner you should have been passing the entire process all along.

	ulong_t offset_by_depth(Process * process, ustd_t pagetype){
		if !(pagetype){ return process->pagetree;}
		ulong_t ret = 512*8;
		ustd_t h = process->code->expected_memory_usage;
	...
	void * vmtree_fetch(Process * process, ustd_t pages_number, ustd_t pagetype){
		...
		for (ustd_t su = 0; su < pagetype; ++su);
			...
			for (ustd_t h = 0; h < 512; ++h){
				...
				if !(tables[su][h] & 1){ --boo;}	//we have a bunch of stacks so i am not stopping it instantly
				if (boo = quota){ break;}
			}
			if (boo == quota){
				tables[su+1] = offset_by_depth(get_process_object(void),su+1) + offsets<<(64-(12+9*(pag::SMALLPAGE-su)>>(12+9*(pag::SMALLPAGE-su)));
			}
			else{
				do {
					offsets |= 1<<(12+(pag::SMALLPAGE-su)*9);
					ustd_t shift = 12+9*(pag::SMALLPAGE-su);
					if (offsets<<(64-shift)>>57 == 511){ --su;}
					else {break;}
				}while (su);
				if !(su){
		...
		for (ustd_t k = 0; k < pagetype; ++k){
			tables[k][offsets<<(12+su*9)>>(64-(12+(pag::SMALLPAGE-k)*9))] |= 1024;
		}
		__non_temporal process->calendar = 0;
		return offsets;
	}
	void vmtree_lay(ulong_t * location, uchar_t expected_usage){	//in gigs
		...
		for (ustd_t h = 0; h < level_len; ++h){
			location[h] = default_entry | (pointer);
		}
	}
	...
	ulong_t * vmto_entry(uint64_t * pagetree, void * vm, ustd_t * pagetype){
		...
		for (ustd_t g = 1; g < pag::SMALLPAGE; ++g){
			ustd_t shift = 12+9*g;
			ret += vm<<(64-shift-9)>>shift * get_multi_from_pagetype(pag::SMALLPAGE)/4096*8;
			if (ret* & 128){
				break;}
		}
		return ret;
	...
	ulong_t * physto_offset(Process * process, void * phys){
		//skipping the higher pagetables
		ulong_t offset = 512*8;
		for (ustd_t j = 0; j < pag::SMALLPAGE; ++j){
			offset += (process->code->expected_memory_usage)*offset;
		}
		//now computing the offset using the pointer
		ulong_t second = 0;
	...
	*/
	...
	void * mem_map(void * pagetree, void * target, ustd_t pagetype, ustd_t pages_number, ustd_t cache){
		...
		for (ustd_t h = 0; h < pages_number; ++h){
			off[h] = templ | ((target+addition*h)<<12);
		}
	}
	void mem_free(void * pagetree, void * vm, ustd_t pages_number, ustd_t pagetype){	//you know that running this on fucking mapped pages marks teh end?
		...
		if (deallocate_vm(get_process_object(void),phys,vm,pages_number)){get_process_object(void)->sigset |= signals::SIGSEGV; return;}
	...
	void mem_unmap(void * pagetree, void * vm, ustd_t pages_number, ustd_t pagetype){
		...
		for (ustd_t h = 0; h < pages_number; ++h){
			...
			if (off[h]<<53>>61 != mode::MAP){ get_process_object(void)->sigset |= SIGSEGV; return 2;}
		}
		for (ustd_t h = 0; h < pages_number; ++h){
	...
	void * mem_realloc(void * pagetree, void * vm, ustd_t oldpages, ustd_t newpages, ustd_t pagetype){	//"Keep your pointer, son. I will take it from here."
		...
		if (deallocate_vm(pagetree,phys,vm,oldpages)){
			get_process_object(void)->sigset |= signals::SIGSEGV;}
	...
	void * mem_remap(void * pagetree, void * vm, ustd_t oldpages, ustd_t newpages, ustd_t pagetype){
		...
		if (deallocate_vm(pagetree,phys,vm,oldpages)){					//this is going to segmentation fault no matter what you do
			get_process_object(void)->sigset |= signals::SIGSEGV; return -1;}
		...
		ustd_t addition = get_multi_from_pagetype(pagetype);
		for (ustd_t h = 0; h < newpages; ++h){
			off[h] = templ | (phys+addition*h)<<12;
		}
		return vm;
	}
	...
	*/
	void swap_process(void){
		...
		ustd_t interval = process->code->expected_memory_usage;
		ustd_t scaling = 512;
		ustd_t pagetype = 0;
		for (ustd_t i = 0; 1; ++i){
	...
	void softfault_handler(Process * process, void * entry, ustd_t pagetype){
		...
		switch(entry* <<(64-12))>>53)
			...
			case mode::MAP:{
				...
				ustd_t page = (entry*)>>46;

				File * file = &vfs->descriptions[process->descs->pool[desc]->findex];
				entry* = vfs->load_page(get_disksking_object(void),file,page) | memreq_template(pagetype,mode::MAP,file->cache,file->executable);
			break;}
			case mode::SEGFAULT:{
				process->sigset |= signals::SIGSEGV;
			break;}
	...
	void hardfault_handler(Process * process, void * entry, ustd_t pagetype){
		...
		if ((entry*<<(64-12))>>54 == mode::SWAPPED){
			void * phys = malloc(1,pagetype);
			void * newentry = this->vmtree_fetch(process->pagetree,1,pagetype);
			newentry = this->memreq_template(pagetype,RESERVE,WRITEBACK,0) | (phys<<5>>18);	//userspace means writeback is fit.
			ustd_t disk = (entry*)>>57;
			void * doffset = (entry*) <<5>>18;
			DisksKing * dking = get_disksking_object(void);
			dking->read(disk,doffset,phys,1,pagetype);
			ustd_t pages = this->get_multi_from_pagetype(pagetype) / 4096;
			Virtual_fs * vfs = get_vfs_object(void);
