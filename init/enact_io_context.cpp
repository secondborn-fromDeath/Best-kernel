...
void enact_IO_context(ustd_t drivers_number, image_loaded ** driver_pointers, image_loaded * info_pointers){
	...
	for (ustd_t i = 0; i < drivers_number; ++i){
		++drivers_number;
		fiDriv * fdriv = vfs->ramcontents_to_description(driver_pointers[i]->image_pointer,driver_pointers[i]->image_size,get_pagetype_from_number(driver_pointers[i]->image_size));
		drivgod->pool_alloc(1)* = fdriv;
		fdriv->d->runtime = kprc->pool[exec(fdriv)];
		fdriv->d->runtime->code->models = vfs->ramcontents_to_description(info_pointers[i]->image_pointer,driver_pointers[i]->image_size,get_pagetype_from_number(info_pointers[i]->image_size));
